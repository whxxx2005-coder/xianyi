      ...
      <main className={currentPage === Page.POSTER ? "" : "pt-14"}>
        {currentPage === Page.POSTER && (
          <div className="fixed inset-0 bg-stone-950 flex flex-col items-center justify-center text-white z-50 overflow-hidden">
            <ImageWithFallback 
              assetKey="poster" 
              cloudUrl="https://images.unsplash.com/photo-1579783902614-a3fb3927b6a5?auto=format&fit=crop&q=80&w=1200" 
              alt="鲜衣怒马少年时" 
              className="absolute inset-0 w-full h-full object-cover opacity-60 scale-105" 
            />
            <div className="relative z-10 text-center px-10">
               <h1 className="text-4xl font-serif font-black tracking-widest mb-4">鲜衣怒马少年时</h1>
               <div className="w-10 h-0.5 bg-[#CF4432] mx-auto mb-6 shadow-lg" />
               ...
               {/* 访客同步码窗口 */}
               <div className="max-w-xs mx-auto bg-white/10 backdrop-blur-2xl p-6 rounded-[2.5rem] border border-white/20 shadow-2xl space-y-4" onClick={(e) => e.stopPropagation()}>
                 <div className="text-[10px] font-black text-white/60 tracking-widest uppercase">
                   {isSyncingFromHome ? '正在激活资源...' : '资源同步激活'}
                 </div>
                 <div className="flex gap-2">
                   ...
                   <button 
                     ...
                     className="bg-[#CF4432] text-white px-5 py-3 rounded-2xl text-[11px] font-black uppercase hover:bg-[#b23a2b] active:scale-95 transition-all shadow-lg min-w-[60px]"
                   >
                     {isSyncingFromHome ? '...' : '确认'}
                   </button>
                 </div>
                 {syncStatusMsg && (
                   <p className="text-[9px] text-white/60 font-bold animate-pulse">{syncStatusMsg}</p>
                 )}
                 {!syncStatusMsg && (
                   <p className="text-[9px] text-white/40 font-bold">注：输入123456同步云端多媒体文件</p>
                 )}
               </div>
               
               <div className="mt-12 opacity-30">
                 <p className="text-[9px] font-black tracking-[0.5em] text-white/50 uppercase">START RESEARCH EXPLORATION</p>
               </div>
            </div>
          </div>
        )}
